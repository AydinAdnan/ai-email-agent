"""Phase 3.6: the constrained feedback parser, the scope echo and the claim ledger.
...
def stored(reading: Reading, answer: str, context: FeedbackContext | None = None) -> object:
    ...
    return ledger.store(claim) if claim is not None else None
...
def test_a_cron_suppression_is_scoped_to_the_mail_it_was_typed_about() -> None:
    ...
    ledger = ClaimLedger()
    assert ledger.stored_claims == 0
    assert reading.prompt is not None
    confirmed = confirm_claim(claim, "yes", context=CRON, recorded_at=NOW)
    assert confirmed is not None
    assert ledger.store(confirmed).route is Route.PROCEED_SILENTLY
    assert ledger.stored_claims == 1
...
def test_an_ambiguous_reading_with_no_mail_in_front_of_it_stores_nothing_on_a_yes() -> None:
    reading = read("ignore everything from now")
    assert reading.claim is not None
    assert reading.claim.scope_anchor is ScopeAnchor.UNRESOLVED
    assert reading.prompt is not None and "Which mail did you mean" in reading.prompt
    # "yes" answers a question about which mail, and names no mail.
    assert confirm_claim(reading.claim, "yes", recorded_at=NOW) is None
    assert stored(reading, "yes") is None


def test_the_answer_can_name_the_scope_the_question_asked_for() -> None:
    reading = read("ignore everything from now")
    narrowed = confirm_claim(reading.claim, "only from status@acme.example", recorded_at=NOW)
...
def test_the_same_claim_twice_is_stored_once() -> None:
    ...
    ledger = ClaimLedger()
    ledger.store(first.claim)
    ledger.store(same.claim)
    assert ledger.stored_claims == 1
